...
function UploadPage({ response, setResponse }) {
  ...
  const renderedFiles = filesArray.map((file, key) => {
    ...
    return (
      <FileNameContainer key={key} filename={name} extension={extension} />
    );
  });
  console.log(files);
  return (
    <>
      ...
      <Canvas>
        ...
        >
          ...
          >
            <div className="flex flex-col items-center justify-center gap-4 p-3">
              ...
              <p className="text-md font-bold">
                drag & drop files or{" "}
                <span className="text-logo underline font-bold">browse</span>
              </p>
          ...
          <button className="px-2 py-3 w-full bg-black rounded-md text-white font-poppins">
            Submit
          </button>
